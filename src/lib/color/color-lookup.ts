import colorIndexMap from '../../data/color-map.json'
import hexColorMap from '../../data/hex-color.json'

export const HexColorLookup: string[] = hexColorMap;
...
export function toColorHex(input : string, xtermOn: boolean) : string | undefined
{
    const index = getColorIndex(input)
    if(index < 0 || index > 256) return undefined
    else if(xtermOn) return HexColorLookup[index]
    else return HexColorLookup[colorIndexMap.XtermLookup[index]]
}
...
export function interpretToken(token: string, xtermOn: boolean): string | TransparentColorPair
{
    ...
    const color: string|undefined = toColorHex(token, xtermOn)
    if(color!== undefined)
    {
    ...
    else if(token[0]==='{' || token[0]==='}')
    {
        ...
        else if(CharacterLookUp[token[1]] !== undefined) return CharacterLookUp[token[1]]
        return ''
    }
    else{
...
export const defaultFg:string = HexColorLookup[7];
export const defaultBg:string = HexColorLookup[0];
